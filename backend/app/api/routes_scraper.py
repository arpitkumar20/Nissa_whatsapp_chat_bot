...
#     results = []
#     namespaces = []
#     merged_records = []  # final list of dicts to write to namespaces.json
#     for url in site_urls:
#         try:
#             web_info, result_path = web_scraping(url)  # Returns Path or None

#             if result_path is None:
#                 raise Exception("Failed to generate embedding data.")

#             # Convert Path to string before passing
#             store_data = store_embeddings_from_folder(str(result_path.parent))
#             # normalize namespace value
#             if isinstance(store_data, dict) and "namespace" in store_data:
#                 namespace_value = store_data["namespace"]
#             else:
#                 namespace_value = str(store_data)

#             # attach namespace into the SAME web_info dict
#             web_info_with_ns = dict(web_info)  # copy to avoid mutating original if needed
#             web_info_with_ns["namespace"] = namespace_value

#             # Save merged record via your COMMON helper (if you want to continue using it)
#             save_data = COMMON.save_json_data(web_info_with_ns)
#             logging.info(f"Website record saved successfully via COMMON.save_json_data: {save_data}")

#             # collect results + objects for final namespaces.json
#             results.append({
...
#             merged_records.append(web_info_with_ns)

#         except Exception as e:
#             logging.exception("Error processing URL %s", url)
#             results.append({
#                 "message": "Failed to scrape data from web site or store it in vectordb",
#                 "url": url,
#                 "error": str(e)
#             })
...
# def async_store_embeddings(result_path, web_info, results, namespaces, merged_records):
#     """
#     Stores embeddings and updates shared lists in a thread-safe manner.
#     """
#     try:
#         # Store embeddings
#         store_data = store_embeddings_from_folder(str(result_path.parent))
...
# def background_scraping(site_urls):
#     """
...
#                 args=(result_path, web_info, results, namespaces, merged_records)
#             )
#             thread.start()
...
#     # Optional: Wait for all threads to complete if needed (can be skipped for true async fire-and-forget)
...
#     Thread(target=background_scraping, args=(site_urls,), daemon=True).start()

#     # Immediately return response to the user
#     return jsonify({
#         "status": "success",
#         "message": "Scraping started in background. You will be notified once complete."
#     })
